...
function App() {
  ...
  const addNonZoomCommandGeneratedOutputToHistory = (data: CommandResponse) => {
    ...
    if (data.result_type === 'enhance_image') {
      const result = data.result as EnhanceResponse

      if (!result.enhanced_filename || !result.enhanced_file_url) {
        return
      }
      ...
      addGeneratedOutputHistoryItem({
        ...
        filename: result.enhanced_filename,
        file_url: result.enhanced_file_url,
        source: result.source ?? 'uploads',
        source_filename: result.filename,
      })
    }
...
const handlePlanAndPrepareExampleCommand = async (exampleCommand: string) => {
  ...
  return (
    <main className="page">
        ...
        <WorkspaceLocalBackupPanel
          workspaceLocalBackupAutoSavedAt={workspaceLocalBackupAutoSavedAt}
          workspaceLocalBackupPreview={workspaceLocalBackupPreview}
          workspaceLocalBackupNotice={workspaceLocalBackupNotice}
          workspaceLocalBackupError={workspaceLocalBackupError}
          hasWorkspaceSnapshotResults={hasWorkspaceSnapshotResults}
          isBusy={isBusy}
          onSaveWorkspaceLocally={handleSaveWorkspaceLocally}
          onLoadLocalWorkspaceSnapshot={handleLoadLocalWorkspaceSnapshot}
          onClearLocalWorkspaceBackup={handleClearLocalWorkspaceBackup}
        />
