...
export function MultiFrameExtractionResultSection({
  ...
  return (
    <section className="card" ref={videoMultiFrameResultRef}>
      <h2>{isDeveloperMode ? 'Multi-Frame Extraction Result' : 'Extracted frames ready'}</h2>
      ...
      <div className="button-row multiframe-detection-actions">
        <button
          ...
          disabled={isBusy || !videoMultiFrameResult}
        >
          {isDetectingMultipleFrames ? 'Detecting frames...' : isDeveloperMode ? 'Run YOLO on Extracted Frames' : 'Detect objects in frames'}
        </button>
      </div>
