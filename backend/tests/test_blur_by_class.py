...
def test_blur_best_object_by_class_success(tmp_path, monkeypatch):
    ...
    response = client.post(
        "/vision/blur-by-class/sample.png",
        json={
            "class_name": "person",
...
def test_blur_best_object_by_class_no_match(tmp_path, monkeypatch):
    test_upload_dir = tmp_path / "uploads"
    test_upload_dir.mkdir(parents=True, exist_ok=True)
...
def test_blur_best_object_by_class_invalid_threshold(tmp_path, monkeypatch):
    test_upload_dir = tmp_path / "uploads"
    test_upload_dir.mkdir(parents=True, exist_ok=True)
